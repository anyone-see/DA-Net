...
class DWAF(nn.Module):
    ...
    def forward(self, Fv, Fa):
        ...
        return att * Fv_ + (1 - att) * Fa_


if __name__ == '__main__':
    cmam = DWAF(input_channels=512, emb_size=512, num_heads=32, dim_head=64, input_size=4,
                 patch_size=1, mlp_channels=1024,
                 att_drop=0.1, ffn_drop=0.1)
    input_fa = torch.randn(2, 512, 8, 8)
    input_fv = torch.randn(2, 512, 8, 8)
    out = cmam(input_fv, input_fa)
    torch.save(cmam.state_dict(), 'cmam.pth')
    # 计算模型的参数量
    print('Number of parameters: ', sum(param.numel() for param in cmam.parameters()))
    # 计算模型的大小
    print('Size of model: ', sum(p.numel() * p.element_size() for p in cmam.parameters()) / (1024 ** 2), 'MB')
    import torchsummary

    torchsummary.summary(cmam, input_size=[(512, 4, 4), (512, 4, 4)], device='cpu')
    print(out.shape)
